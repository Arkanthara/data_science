# Espaces de Représentation en Haute Dimension
...
**Data Science = ML** : Étude des espaces de représentation où agit le ML. **Pipeline** : Processus → Capteur → Données brutes → Représentation (matrice dans $\mathbb{R}^D$). Focus sur propriétés géométriques ET statistiques.
:::
...
$X = \{x_1, \ldots, x_N\}$ avec $x_i \in \Omega \subseteq \mathbb{R}^D$. **Deux visions** : (1) Statistique : échantillon de $f_X$, variables i.i.d. (2) Algébrique : matrice $X \in \mathbb{R}^{D \times N}$. **Structure** : espace vectoriel → produit scalaire → norme → distance → espace métrique.
:::
...
Pour estimer une densité avec qualité, besoin de $N \approx n \cdot b^D$ échantillons (croissance **exponentielle**). Si $N$ fixé : $n = N/b^D$ → la plupart des bins **vides**. Exemple : $D=6$, $b=10$, $n=10$ → $N \approx 10^7$.
:::
...
**Ratio sphère/cube** : $\frac{\text{vol}(B_D(r))}{\text{vol}(C_D(r))} = \frac{\pi^{D/2}}{D \cdot 2^{D-1} \Gamma(D/2)} \to 0$. Volume concentré aux **coins** du cube (distance centre-coin = $\sqrt{D}/2$). Échantillonnage uniforme → sphère **vide**.
:::
...
Pour **toute forme** : ratio de volumes $(1-\varepsilon)^D \to 0$ quand $D \to \infty$. **Méthode** : décomposition en voxels hypercubiques. **Résultat** : le volume est **concentré à la surface** (différence $\to 1$).
:::
...
Pour $X \sim \mathcal{N}(0, \text{Id}_D)$ : $R^2 = \|X\|_2^2 \sim \chi^2(D)$ donc $\mathbb{E}[R^2] = D$. **Gaussian Annulus Theorem** : $P[|\|X\|_2 - \sqrt{D}| > \beta] \leq 3e^{-c\beta^2}$. Densité concentrée au rayon $\sigma\sqrt{D}$ (anneau fin).
:::
...
Si variance de distance normalisée $\to 0$, alors $\frac{D_{\max} - D_{\min}}{D_{\min}} \to 0$. Tous les voisins sont **équidistants**. KNN et $\varepsilon$NN **perdent leur sens**.
:::
...
**2 vecteurs aléatoires** : $\mathbb{E}[\cos(\theta)] = 0$ → $\theta \sim \pi/2$ (quasi-orthogonaux). **Triangle aléatoire** : $\mathbb{E}[\cos(\theta)] = 1/2$ → $\theta \sim \pi/3$ (équilatéral). **Sphère** : volume à l'équateur.
:::
...
$n_k(x_j)$ = nombre de fois où $x_j$ est k-NN d'un autre point. Distribution **asymétrique** : quelques **hubs** apparaissent dans beaucoup de voisinages. Ces hubs sont "proches" de tous.
:::
...
**Hoeffding** : $P(|\bar{X} - \mu| > \varepsilon) \leq 2e^{-2D\varepsilon^2/(b-a)^2}$. Agrégation de variables indépendantes → convergence vers $\mu$. Structure $\sum_{i=1}^D (\cdot)$ dans distances → **concentration des distances**.
:::
...
**WLLN** : $\bar{X} \xrightarrow{P} \mu$ quand $D \to \infty$. Base de l'estimation de densité. **CLT** : $Z_D = \frac{\sqrt{D}}{\sigma}(\bar{X} - \mu) \sim \mathcal{N}(0, 1)$ (taux de convergence).
:::
...
**Principe** : Exponentiellement nombreuses directions quasi-orthogonales en haute dimension. **Johnson-Lindenstrauss** : projection $\mathbb{R}^D \to \mathbb{R}^d$ préservant distances (à $\varepsilon$ près) avec $d = O(\log N / \varepsilon^2)$ (indépendant de $D$ !). Application : LSH pour ANN.
:::
...
**Échantillonnage** : besoin exponentiel de samples, taux de rejet → 100%. **Indexation** : structure disparaît, kd-trees → complexité $O(N)$, hubness. **Learning** : $N \geq \varepsilon^{-D} D^{D/2} / (2\pi e)^{D/2}$ → impossible pour $D$ grand et $\varepsilon$ petit.
:::
...
Analyses supposent distribution **uniforme** et features **indépendantes**. **Réalité** : données sur **sous-espaces** de dimensionnalité intrinsèque plus faible. **Solutions** : décorrélation, clustering, réduction de dimension.
:::
...
**Haute dimension** ($D \gtrsim 10$) ≠ géométrie 3D. **⊕** : inégalités de concentration pour estimation, projections aléatoires. **⊖** : data sparsity, concentration distances/angles, hubs. **Solution** : opérer en **basse dimension**.
:::
